...
其中 $M$ 表示只保留部分频谱样本等价于在 k 空间元素逐点相乘， $\eta$ 为噪声。
...
②基于 k 空间插值的GRAPPA（GeneRalized Autocalibrating Partial Parallel Acquisition）：在 k 空间用邻域整形算子插值未采样的 k 点，系数由自校正信号（ACS）估计。
