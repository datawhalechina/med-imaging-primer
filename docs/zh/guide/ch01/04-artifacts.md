...
::: warning ⚠️ 临床注意
运动伪影常见于腹部扫描（呼吸运动）和心脏扫描（心跳）。在评估小病灶时，需要区分真实病变和运动伪影。多层面观察和对比增强前后扫描有助于鉴别。低螺距扫描（pitch < 1.0）可以获得更多的重叠采样，但也会延长扫描时间，因此是否有助于减少运动伪影取决于具体部位、扫描协议和患者配合情况。
:::
...
- **增加接收带宽**：
  ...
  - 化学位移像素数 = 化学位移频率 / 每像素接收带宽（bandwidth per pixel）
...
- **特殊形式**：
  ...
  - **环降伪影（Ring-Down）**：常与气泡或气体共振相关的连续高回声带
