...
| **Print Management Service Class** | N-CREATE / N-SET / N-ACTION | 创建胶片会话、设置打印参数并执行打印 |
...
![人体解剖平面示意](https://upload.wikimedia.org/wikipedia/commons/2/24/Human_anatomy_planes%2C_labeled.svg)
*人体解剖平面示意：医学影像中的世界坐标最终要对应到左右、前后、上下等解剖方向；DICOM、NIfTI和不同软件的坐标约定不同，因此转换时必须检查方向矩阵。*

**坐标转换方法：**

NIfTI提供两种方法将体素坐标转换为世界坐标：

1. **Method 1（qform）**：基于四元数的空间变换
   - 适用于扫描仪坐标系
   - 包含旋转、平移、体素尺寸缩放和方向符号（qfac），但不表达任意剪切变换
...
- 人体解剖平面示意：[Wikimedia Commons - Human anatomy planes, labeled](https://commons.wikimedia.org/wiki/File:Human_anatomy_planes,_labeled.svg)，CC BY-SA 4.0
